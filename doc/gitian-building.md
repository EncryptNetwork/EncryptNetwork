...
*Setup instructions for a gitian build of XCX using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the EncryptCore
...
- [Building EncryptCore](#building-encrypt)
...
Clone the git repositories for encrypt and gitian and then checkout the encrypt version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/encrypt-crypto/encrypt.git
cd encrypt
...
Follow the instructions in [doc/release-process.md](release-process.md) in the encrypt repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building EncryptCore
----------------

To build XCX (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the encrypt repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/encrypt/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/encrypt-crypto/encrypt
...
For example:
```bash
URL=https://github.com/crowning-/encrypt.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit encrypt=${COMMIT} --url encrypt=${URL} ../encrypt/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit encrypt=${COMMIT} --url encrypt=${URL} ../encrypt/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit encrypt=${COMMIT} --url encrypt=${URL} ../encrypt/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/encrypt-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/encrypt-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/encrypt-build.assert
```
...
[encrypt/gitian.sigs](https://github.com/encrypt-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
