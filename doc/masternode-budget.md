...
Encryptcoin now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<encrypt_address\> \<monthly_payment_encrypt\> [use_ix(true|false)]
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ EncryptCore, _12_ times over the course of a year totaling _24000_ EncryptCore.
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<encrypt_address\> \<monthly_payment_encrypt\> \<collateral_hash\>
...
When block `1000000` is reached you'll receive a payment for `1200` EncryptCore.
 ...
 * vote-many          - Vote on a Encryptcoin initiative
 * vote-alias         - Vote on a Encryptcoin initiative
 * vote               - Vote on a Encryptcoin initiative/budget
