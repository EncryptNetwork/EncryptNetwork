...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("EncryptCoreMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("EncryptCoreMiner : ProcessNewBlock, block not accepted");
...
bool fGenerateEncryptcoins = false;
...
void BitcoinMiner(CWallet* pwallet, bool fProofOfStake, MineType mineType)
{
    ...
    LogPrintf("EncryptCoreMiner started with %sms sleep time\n", nDefaultMinerSleep);
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("encrypt-miner");
    ...
    while (fGenerateEncryptcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            while (vNodes.empty() || pwallet->IsLocked() || !fMintableCoins || nReserveBalance >= pwallet->GetBalance() || !masternodeSync.IsSynced()) {
                ...
                MilliSleep(5000);
                if (!fGenerateEncryptcoins) {
                    break;
                }
                if (!fGenerateEncryptcoins && !fProofOfStake)
                    continue;
            }
        ...
        LogPrintf("Running EncryptCoreMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
            ::GetSerializeSize(*pblock, SER_NETWORK, PROTOCOL_VERSION));
...
void static ThreadEncryptcoinMiner(void* parg)
{
    boost::this_thread::interruption_point();
    try {
...
void GeneratePoAEncryptcoin(CWallet* pwallet, int period)
{
    ...
    minerThreads->create_thread(boost::bind(&ThreadEncryptcoinMiner, pwallet));
}

void GenerateEncryptcoins(bool fGenerate, CWallet* pwallet, int nThreads)
{
    static boost::thread_group* minerThreads = NULL;
    fGenerateEncryptcoins = fGenerate;
