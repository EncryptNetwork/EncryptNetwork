...
void BitcoinMiner(CWallet* pwallet, bool fProofOfStake, MineType mineType)
{
    ...
    while (fGenerateEncrypts || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            while (vNodes.empty() || pwallet->IsLocked() || !fMintableCoins || nReserveBalance >= pwallet->GetBalance() || !masternodeSync.IsSynced()) {
                ...
                MilliSleep(5000);
                if (!fGenerateEncrypts) {
                    break;
                }
                if (!fGenerateEncrypts && !fProofOfStake)
                    continue;
            }
...
void static ThreadEncryptMiner(void* parg)
{
    boost::this_thread::interruption_point();
    try {
...
void GeneratePoAEncrypt(CWallet* pwallet, int period)
{
    ...
    minerThreads->create_thread(boost::bind(&ThreadEncryptMiner, pwallet));
}

void GenerateEncrypts(bool fGenerate, CWallet* pwallet, int nThreads)
{
    static boost::thread_group* minerThreads = NULL;
    fGenerateEncrypts = fGenerate;
