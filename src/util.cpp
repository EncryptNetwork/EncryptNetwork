...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "encrypt" is a composite category enabling all DAP-related debug output
            if (ptrCategory->count(string("encrypt"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\EncryptCore
// Windows >= Vista: C:\Users\Username\AppData\Roaming\EncryptCore
// Mac: ~/Library/Application Support/EncryptCore
// Unix: ~/.encrypt
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EncryptCore";
#else
    fs::path pathRet;
    ...
    return pathRet / "EncryptCore";
#else
    // Unix
    return pathRet / ".encrypt";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "encrypt.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty encrypt.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override encrypt.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "encryptd.pid"));
    ...
    if (boost::filesystem::is_directory(boost::filesystem::status(pathPidFile)) ||
        ...
        pathPidFile = GetDataDir() / boost::filesystem::path("encryptd.pid");
