    ...
    if (!tx.IsCoinBase()) {
        for (unsigned int i = 0; i < tx.vin.size(); i++) {
            ...
            for (size_t j = 0; j < alldecoys.size(); j++) {
                ...
                //UTXO with 1M XCX can only be consumed in a transaction with that single UTXO
...
CAmount TeamRewards(const CBlockIndex* ptip)
{
    ...
    if (lastPoABlock->hashPrevPoABlock.IsNull()) {
        //pay encrypt team after the first PoA block
        return (pForkTip->nHeight - Params().LAST_POW_BLOCK() - 1 + 1 /*+1 for the being created PoS block*/) * 100 * COIN;
    }

    //loop back to find the PoA block right after which the encrypt team is paid
...
bool RecalculateXCXSupply(int nHeightStart)
{
    const int chainHeight = chainActive.Height();
