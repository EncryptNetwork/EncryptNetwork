...
#if defined(NDEBUG)
#error "XCX cannot be compiled without assertions."
#endif
...
bool VerifyShnorrKeyImageTx(const CTransaction& tx)
{
    ...
    //this assumes that the transaction is already checked for either a staking transaction or transactions spending only UTXOs of 1M XCX
...
CAmount TeamRewards(const CBlockIndex* ptip)
{
    ...
    if (lastPoABlock->hashPrevPoABlock.IsNull()) {
        //pay XCX team after the first PoA block
        return (pForkTip->nHeight - Params().LAST_POW_BLOCK() - 1 + 1 /*+1 for the being created PoS block*/) * 100 * COIN;
    }

    //loop back to find the PoA block right after which the XCX team is paid
    ...
    if (!lastPoAHash.IsNull() && numPoABlocks != 0 && numPoABlocks % 24 == 0) {
        ret = (pForkTip->nHeight - (mapBlockIndex[lastPoAHash]->nHeight + 1) - numPoABlocks + 1 /*+1 for the being created PoS block*/) * 100 * COIN;
    }
    return ret;
}
...
{
    util::ThreadRename("encrypt-scriptch");
    scriptcheckqueue.Thread();
}

bool RecalculateENCRYPTSupply(int nHeightStart)
{
    const int chainHeight = chainActive.Height();
    ...
    uiInterface.ShowProgress(_("Recalculating XCX supply..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainHeight - nHeightStart))));
            uiInterface.ShowProgress(_("Recalculating XCX supply..."), percent);
        }
        CBlock block;
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        //check foundation wallet address is receiving 50 XCX
        const CTransaction& coinstake = block.vtx[1];
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        if (pfrom->strSubVer == "/EncryptCore:0.27.5.1/" || pfrom->strSubVer == "/EncryptCore:1.0.1/" || pfrom->strSubVer == "/XCX:1.0.1.3/" || pfrom->strSubVer == "/XCX:1.0.2/" || pfrom->strSubVer == "/XCX:1.0.3.4/" || pfrom->strSubVer == "/XCX:1.0.4.6/") {
