...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::XCX)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, QString& sENCRYPTPercentage)
{
    int nPrecision = 2;

    double dPercentage = 100.0;
    
    sENCRYPTPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
}
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("XCX")
    updateDisplayUnit();
...
void OverviewPage::showBalanceSync(bool fShow){
        ...
        if (isSyncingBalance){
            QString tooltip = "The displayed information may be out of date. Your wallet automatically synchronizes with the XCX network after a connection is established, but this process has not completed yet.";
...
void OverviewPage::showBlockSync(bool fShow)
{
    ...
    if (isSyncingBlocks){
        ...
        ui->labelBlockStatus->setToolTip("The displayed information may be out of date. Your wallet automatically synchronizes with the XCX network after a connection is established, but this process has not completed yet.");
        ui->labelBlockCurrent->setAlignment((Qt::AlignRight|Qt::AlignVCenter));
    } else {
        ui->labelBlockStatus->setText("(synced)");
        ui->labelBlockStatus->setToolTip("Your wallet is fully synchronized with the XCX network.");
