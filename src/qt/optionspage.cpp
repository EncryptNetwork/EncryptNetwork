...
void OptionsPage::on_EnableStaking(ToggleButton* widget)
{
    ...
    if (widget->getState()){
        ...
        if (stt == StakingStatusError::UNSTAKABLE_BALANCE_TOO_LOW || 
            ...
            QMessageBox msgBox;
            if (stt == StakingStatusError::UNSTAKABLE_BALANCE_TOO_LOW) {
                errorMessage = "Your stakeable balance is under the threshold of 1 000 XCX. Please deposit more XCX into your account in order to enable staking.";
                ...
                errorMessage = "Your stakeable balance is under the threshold of 1 000 XCX. This is due to your reserve balance being too high. Please deposit more XCX into your account or reduce your reserved amount in order to enable staking.";
            } else {
                CAmount totalFee = maxFee + pwalletMain->ComputeFee(1, 2, MAX_RING_SIZE);
                errorMessage = "Your stakeable balance is under the threshold of 1 000 XCX. This is due to your reserve balance of " + FormatMoney(nReserveBalance) + " XCX being too high. The wallet software has tried to consolidate your funds with the reserve balance but without success because of a consolidation fee of " + FormatMoney(totalFee) + " XCX. Please wait around 10 minutes for the wallet to resolve the reserve to enable staking.";
            }
