                                                          ...
                                                          mapper(0)
{
    ...
    ui->lineEditWithhold->setPlaceholderText("XCX Amount");
    ...
    if (!pwalletMain->IsMasternodeController()) {
        ...
        ui->addNewFunds->setToolTip("Disabled by default due to controlling Masternode(s) from this wallet.\nEnabling this will incur a minimum 1 XCX fee each time you receive a new deposit that needs to be consolidated for staking.");
    }
...
{
    ui->toggleStaking->setState(fGenerateEncrypts);
}
...
void OptionsPage::on_pushButtonSave_clicked() {
    ...
    if (ui->lineEditWithhold->text().trimmed().isEmpty()) {
        ...
        msgBox.setText("XCX reserve amount is empty and must be a minimum of 1.\nPlease click Disable if you would like to turn it off.");
    ...
    msgBox.setText("Reserve balance of " + reserveBalance + " XCX is successfully set.");
    msgBox.setStyleSheet(GUIUtil::loadStyleSheet());
    msgBox.setIcon(QMessageBox::Information);
    msgBox.exec();
}
...
void OptionsPage::on_EnableStaking(ToggleButton* widget)
{
    ...
    if (widget->getState()){
        ...
        if (stt == StakingStatusError::UNSTAKABLE_BALANCE_TOO_LOW || 
            ...
            QMessageBox msgBox;
            if (stt == StakingStatusError::UNSTAKABLE_BALANCE_TOO_LOW) {
                errorMessage = "Your stakeable balance is under the threshold of 400 000 XCX. Please deposit more XCX into your account in order to enable staking.";
            } else if (stt == UNSTAKABLE_BALANCE_TOO_LOW_CONSOLIDATION_FAILED) {
                errorMessage = "Your balance requires a consolidation transaction which incurs a fee of between  " + FormatMoney(minFee) + " to " + FormatMoney(maxFee) + " XCX. However after that transaction fee, your balance will be below the staking threshold of 400 000 XCX. Please deposit more XCX into your account or reduce your reserved amount in order to enable staking.";
            } else if (stt == UNSTAKABLE_BALANCE_RESERVE_TOO_HIGH) {
                errorMessage = "Your stakeable balance is under the threshold of 400 000 XCX. This is due to your reserve balance being too high. Please deposit more XCX into your account or reduce your reserved amount in order to enable staking.";
            } else {
                CAmount totalFee = maxFee + pwalletMain->ComputeFee(1, 2, MAX_RING_SIZE);
                errorMessage = "Your stakeable balance is under the threshold of 400 000 XCX. This is due to your reserve balance of " + FormatMoney(nReserveBalance) + " XCX being too high. The wallet software has tried to consolidate your funds with the reserve balance but without success because of a consolidation fee of " + FormatMoney(totalFee) + " XCX. Please wait around 10 minutes for the wallet to resolve the reserve to enable staking.";
            }
        ...
        if (stt == StakingStatusError::STAKABLE_NEED_CONSOLIDATION) {
            errorMessage = "In order to enable staking with 100% of your current balance, your previous XCX deposits must be consolidated and reorganized. This will incur a fee of between " + FormatMoney(minFee) + " to " + FormatMoney(maxFee) + " XCX.\n\nWould you like to do this?";
        } else {
            errorMessage = "In order to enable staking with 100% of your current balance except the reserve balance, your previous XCX deposits must be consolidated and reorganized. This will incur a fee of between " + FormatMoney(minFee) + " to " + FormatMoney(maxFee) + " XCX.\n\nWould you like to do this?";
        }
