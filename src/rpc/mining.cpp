...
UniValue getgenerate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or encrypt.conf setting gen)\n"
...
UniValue setgenerate(const UniValue& params, bool fHelp)
{
    ...
    } else // Not -regtest: start generate thread, return immediately
    {
        ...
        GenerateEncryptcoins(fGenerate, pwalletMain, nGenProcLimit);
    }
...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "XCX is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "XCX is downloading blocks...");
