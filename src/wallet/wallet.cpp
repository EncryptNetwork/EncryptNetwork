...
CAmount CWalletTx::GetUnlockedCredit() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        if (fMasterNode && pwallet->getCTxOutValue(*this, vout[i]) == 500000 * COIN) continue; // do not count MN-like outputs
...
CAmount CWalletTx::GetLockedCredit() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        // Add masternode collaterals which are handled likc locked coins
         else if (fMasterNode && pwallet->getCTxOutValue(*this, vout[i]) == 500000 * COIN) {
            nCredit += pwallet->GetCredit(*this, txout, ISMINE_SPENDABLE);
        }
    ...
    cannotSpend = 0;
    {
        ...
        for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
            ...
            if (nCoinType == ONLY_DENOMINATED) {
                ...
                found = !(fMasterNode && value == 500000 * COIN);
                ...
                found = !IsDenominatedAmount(value);
                if (found && fMasterNode) found = value != 500000 * COIN; // do not use Hot MN funds
            } else if (nCoinType == ONLY_500000) {
                found = value == 500000 * COIN;
            } else {
            ...
            if (mine == ISMINE_WATCH_ONLY)
                continue;
            if (IsLockedCoin(wtxid, i) && nCoinType != ONLY_500000)
                continue;
            ...
            while (true) {
                ...
                if (!SelectCoins(true, estimatedFee, 10, 2, nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 XCX.");
                    } else {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-encrypt-address
                    CScript scriptChange;
    ...
    static uint256 reserveHash;
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            if (stakingMode != StakingMode::STAKING_WITH_CONSOLIDATION && (vCoins.empty() || (vCoins.size() < MIN_TX_INPUTS_FOR_SWEEPING) || (total < target + estimatedFee && vCoins.size() <= MAX_TX_INPUTS))) {
                ...
                LogPrintf("Generating consolidation transaction, total = %d XCX\n", total);
                // Generate transaction public key
                CWalletTx wtxNew;
                CKey secret;
                secret.MakeNewKey(true);
...
void CWallet::AutoCombineDust()
{
    ...
    if (stakingMode == StakingMode::STAKING_WITH_CONSOLIDATION) {
        if (IsLocked()) return;
        if (fGenerateEncryptcoins && chainActive.Tip()->nHeight >= Params().LAST_POW_BLOCK()) {
